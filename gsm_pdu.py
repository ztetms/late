...
class PDU():
	...
	@classmethod
	def decode_csca(cls, data):
		return cls.decode_address(data[1:]) if data[0] > 0 else None

	@classmethod
	def parse(cls, string):
		'''
		>>> csca, tpdu = PDU.parse('0891683108501955F1040D91683118140276F8000841011251018123044F60597D')
		>>> csca
		'+8613800591551'
		>>> tpdu.oa
		'+8613814120678'
		...
		csca_length = data[0]
		csca = cls.decode_csca(data[:1 + csca_length])
...
class USER_DATA():
	...
	def factory(type):
		return {
			PDU.DSC_7BIT: UD_7BIT,
			PDU.DSC_8BIT: UD_8BIT,
			PDU.DSC_UCS2: UD_UCS2,
			}[type]
...
class SMS_DELIVER(TPDU):
	...
	@classmethod
	def parse(cls, string):
		return cls.decode(PDU.parse_hex_string(string))
	...
	def check(cls, data):
		...
		dcs = data[2 + oa]
		ud = data[10 + oa]
		ud_decoder = USER_DATA.factory(dcs)
	...
	@staticmethod
	def decode_address(data):
		return PDU.decode_address(data[1:])
